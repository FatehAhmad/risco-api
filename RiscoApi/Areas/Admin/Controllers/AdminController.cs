...
namespace WebApplication1.Areas.Admin.Controllers
{
    ...
    public class AdminController : ApiController
    {
        ...
        //        model.Password = CryptoHelper.Hash(httpRequest.Params["Password"]);
        //        model.ImageUrl = httpRequest.Params["ImageUrl"];

        //        model.Status = (int)DAL.CustomModels.Enumeration.StatusCode.NotVerified;
        ...
        public async Task<IHttpActionResult> GetAdminDashboardStats()
        {
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    ...
                    {
                        ActiveUsers = ctx.Users.Count(x => !x.IsDeleted),
                        ActiveGroups = ctx.Groups.Count(x => x.IsDeleted == false),
                        ActivePosts = ctx.Posts.Count(x => !x.IsDeleted),
                        //TodayOrders = ctx.Orders.Count(x => DbFunctions.TruncateTime(x.OrderDateTime) == TodayDate.Date),
                        DeviceUsage = ctx.Database.SqlQuery<DeviceStats>("select Platform,COUNT(Id) AS Count from UserDevices group by Platform ").ToList()
                    };


                    //var userDevice = ctx.UserDevices.ToList();

                    //if(userDevice != null)
                    //{
                    //    foreach (var item in userDevice.GroupBy(x=>x.Platform))
                    //    {
                    //        model.DeviceUsage.Add(new DeviceStats
                    //        {
                    //            Count = userDevice.Count(x => x.Platform == item.FirstOrDefault().Platform),
                    //            Percentage = item.Count() / userDevice.Count() * 100,
                    //            Platform= item.FirstOrDefault().Platform
                    //        });
...
//        }
        ...
        //                    break;
        //                case (int)RiscoEntityTypes.FAQ:
        //                    ctx.FAQ.FirstOrDefault(x => x.Id == Id).IsDeleted = true;
        //                    break;
        //                default:
        //                    break;
        //            }
...
//        }
        ...
        //                            Notification.Description = "Your order# " + existingStoreOrder.Order.Id + " has been accepted by store.";
        //                            pushType = PushNotificationType.OrderAccepted;
        //                            existingStoreOrder.Order.User.SendingUserNotifications.Add(Notification);
        //                        }
        //                        else if (order.Status == (int)OrderStatuses.AssignedToDeliverer)
        //                        {
        //                            Notification.Title = "Order Assigned To Delivery Boy";
        //                            Notification.Description = "Your order#" + existingStoreOrder.Order.Id + " has been assigned to delivery boy.";
        ...
        //                            Notification.Description = "Your order#" + existingStoreOrder.Order.Id + " has been dispatched.";
        ...
        //                            Notification.Description = "Your order#" + existingStoreOrder.Order.Id + " has been marked as completed.";
        ...
        //                            Notification.Description = "Your order#" + existingStoreOrder.Order.Id + " has been rejected by store, we are very sorry for inconvenience.";
        ...
        //                        {
        //                            var usersToPushAndroid = existingStoreOrder.Order.User.UserDevices.Where(x => x.Platform == (int)DeviceTypes.Android).ToList();
        //                            var usersToPushIOS = existingStoreOrder.Order.User.UserDevices.Where(x => x.Platform == (int)DeviceTypes.IOS).ToList();
            ...
            try
            {
                ...
                using (RiscoContext ctx = new RiscoContext())
                {
                    ...
                    if (model.TargetAudience == (int)NotificationTargetAudienceTypes.User || model.TargetAudience == (int)NotificationTargetAudienceTypes.UserAndDeliverer)
                    {
                        ...
                        await users.ForEachAsync(a => a.SendingUserNotifications.Add(new Notification { Title = model.Title, Description = model.Description, Status = 0, AdminNotification_Id = adminNotification.Id }));

                        await ctx.SaveChangesAsync();

                        var usersToPushAndroid = users.Where(x => x.IsNotificationsOn).SelectMany(x => x.UserDevices.Where(x1 => x1.Platform == (int)DeviceTypes.Android)).ToList();
                        var usersToPushIOS = users.Where(x => x.IsNotificationsOn).SelectMany(x => x.UserDevices.Where(x1 => x1.Platform == (int)DeviceTypes.IOS)).ToList();
            ...
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    ...
                    foreach (var user in model.Users)
                    {
                        ...
                        {
                            case true:
                                var userModel = ctx.Users.FirstOrDefault(x => x.Id == user.UserId);

                                query = @"Update Posts SET IsDeleted='1' Where User_Id='" + user.UserId + @"';
                                      Update Comments SET IsDeleted='1' Where User_Id='" + user.UserId + @"';
                                      Update Likes SET IsDeleted='1' Where User_Id='" + user.UserId + @"';
                                      Update Groups SET IsDeleted='1' Where User_Id='" + user.UserId + @"';
                                      Update GroupMembers SET IsDeleted='1' Where User_Id='" + user.UserId + @"';
                                      Update Shares SET IsDeleted='1' Where User_Id='" + user.UserId + @"';
                                      Update FollowFollowers SET IsDeleted='1' Where FirstUser_Id='" + user.UserId + @"' OR SecondUser_Id='" + user.UserId + @"';
                                      Update Notifications SET IsDeleted='1' Where SendingUser_Id='" + user.UserId + @"' OR ReceivingUser_Id='" + user.UserId + @"'
                                      Update ActivityLogs SET IsDeleted='1' Where FirstUser_Id='" + user.UserId + @"' OR SecondUser_Id='" + user.UserId + @"'
                                      Update Users SET IsDeleted='1' Where ID='" + user.UserId + @"'";

                                if (userModel.ReportCount >= 10)
                                {
                                    userModel.ReportCount = 0;
                                    ctx.SaveChanges();
                                }

                                break;
                            case false:
                                query = @"Update Posts SET IsDeleted='0' Where User_Id='" + user.UserId + @"';
                                      Update Comments SET IsDeleted='0' Where User_Id='" + user.UserId + @"';
                                      Update Likes SET IsDeleted='0' Where User_Id='" + user.UserId + @"';
                                      Update Groups SET IsDeleted='0' Where User_Id='" + user.UserId + @"';
                                      Update GroupMembers SET IsDeleted='0' Where User_Id='" + user.UserId + @"';
                                      Update Shares SET IsDeleted='0' Where User_Id='" + user.UserId + @"';
                                      Update FollowFollowers SET IsDeleted='0' Where FirstUser_Id='" + user.UserId + @"' OR SecondUser_Id='" + user.UserId + @"';
                                      Update Notifications SET IsDeleted='0' Where SendingUser_Id='" + user.UserId + @"' OR ReceivingUser_Id='" + user.UserId + @"'
                                      Update ActivityLogs SET IsDeleted='0' Where FirstUser_Id='" + user.UserId + @"' OR SecondUser_Id='" + user.UserId + @"'
                                      Update Users SET DeActive='0',IsDeleted='0' Where ID='" + user.UserId + @"'";
                                break;
                            default:
                                break;
                        }
        ...
        [HttpGet]
        [Route("GetUserData")]
        public async Task<IHttpActionResult> GetUserData(int User_Id)
        {
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    SearchUsersViewModel returnModel = new SearchUsersViewModel();

                    returnModel.Followers = ctx.FollowFollowers.Where(x => x.SecondUser_Id == User_Id && !x.IsDeleted).Select(x => x.FirstUser).ToList();
                    returnModel.Followings = ctx.FollowFollowers.Where(x => x.FirstUser_Id == User_Id && !x.IsDeleted).Select(x => x.SecondUser).ToList();
                    returnModel.Groups = ctx.Groups.Include(x => x.GroupCategories).Where(x => x.User_Id == User_Id && !x.IsDeleted).ToList();
                    returnModel.Medias = ctx.Medias.Where(x => x.User_Id == User_Id).ToList();
                    returnModel.User = ctx.Users.Include(x => x.Posts).Include(x => x.TrendLogs).Include(x => x.Likes).Include(x => x.Shares).Include(x => x.UserGroups).Include(x => x.FirstUserFollowFollower).Include(x => x.SecondUserFollowFollower).FirstOrDefault(x => x.Id == User_Id);
                    returnModel.ReportUsers = ctx.ReportUsers.Include(x => x.FirstUser).Include(x => x.SecondUser).Where(x => x.FirstUser_Id == User_Id).ToList();
                    returnModel.JoinedGroups = ctx.GroupMembers.Include(x => x.Group).Include(x => x.Group.GroupCategories).Where(x => x.User_Id == User_Id && !x.IsDeleted).Select(x => x.Group).ToList();
                    returnModel.UserDetails.TotalPosts = returnModel.User.Posts.Count(x => !x.IsDeleted);
                    returnModel.UserDetails.TotalGroups = returnModel.User.UserGroups.Count(x => !x.IsDeleted);
                    returnModel.UserDetails.TotalFollowers = returnModel.User.FirstUserFollowFollower.Count(x => !x.IsDeleted);
                    returnModel.UserDetails.TotalFollowings = returnModel.User.SecondUserFollowFollower.Count(x => !x.IsDeleted);
                    returnModel.UserDetails.TotalShares = returnModel.User.Shares.Count(x => !x.IsDeleted);
                    returnModel.UserDetails.TotalLikes = returnModel.User.Likes.Count(x => !x.IsDeleted);
        ...
        [HttpGet]
        [Route("GetGroupData")]
        public async Task<IHttpActionResult> GetGroupData(int Group_Id)
        {
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    GroupDataViewModel returnModel = new GroupDataViewModel();

                    returnModel.Group = ctx.Groups.Include(x => x.User).Include(x => x.GroupMembers).FirstOrDefault(x => x.Id == Group_Id);

                    returnModel.Group.GroupMembers = returnModel.Group.GroupMembers.Where(x => !x.IsDeleted && x.Status == (int)GroupMemberStatusTypes.Accepted).ToList();

                    foreach (var item in returnModel.Group.GroupMembers)
                    {
                        returnModel.GroupMembers.Add(ctx.Users.FirstOrDefault(x => x.Id == item.User_Id));
                    }

                    return Ok(new CustomResponse<GroupDataViewModel>
                    {
                        Message = Global.ResponseMessages.Success,
                        StatusCode = (int)HttpStatusCode.OK,
                        Result = returnModel
        ...
        public async Task<IHttpActionResult> GetUsers()
        {
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    return Ok(new CustomResponse<SearchUsers>
                    {
                        Message = Global.ResponseMessages.Success,
                        StatusCode = (int)HttpStatusCode.OK,
                        Result = new SearchUsers
                        {
        ...
        //            {
        //                var user = ctx.Users.Include(x => x.SendingUserNotifications.Select(x1 => x1.AdminNotification)).Include(x => x.Orders).Include(x => x.UserSubscriptions.Select(y => y.Box)).Include(x => x.Favourites.Select(y => y.Product)).Include(x => x.Feedback.Select(z => z.Store)).Include(x => x.UserAddresses).Include(x => x.PaymentCards).FirstOrDefault(x => x.Id == UserId);
        ...
        public async Task<IHttpActionResult> SendNotificationToUser(SendNotificationToUserBindingModel model)
        {
            try
            {
                ...
                using (RiscoContext ctx = new RiscoContext())
                {
                    ...
                    var androidDevices = users.Where(x => x.IsNotificationsOn).SelectMany(x => x.UserDevices.Where(x1 => x1.Platform == (int)DeviceTypes.Android)).ToList();
                    var iosDevices = users.Where(x => x.IsNotificationsOn).SelectMany(x => x.UserDevices.Where(x1 => x1.Platform == (int)DeviceTypes.IOS)).ToList();
        ...
        public async Task<IHttpActionResult> TestNotificationToAdmin(string Text)
        {
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    var adminsToPush = ctx.UserDevices.Where(x => x.Platform == (int)DeviceTypes.Web).ToList();
                    ...
                    HostingEnvironment.QueueBackgroundWorkItem(cancellationToken =>
                    {
                        Global.objPushNotifications.SendWebGCMPushNotification(adminsToPush, null, notify);
                    });
                }
        ...
        public async Task<IHttpActionResult> GetReportedUsers(int? User_Id)
        {
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    ...
                    if (User_Id.HasValue)
                    {
                        return Ok(new CustomResponse<ReportUsersViewModel>
                        {
                            Message = Global.ResponseMessages.Success,
                            StatusCode = (int)HttpStatusCode.OK,
                            Result = new ReportUsersViewModel
                            {
                                ReportUsers = ctx.ReportUsers.Include(x => x.FirstUser).Include(x => x.SecondUser).Where(x => x.FirstUser_Id == User_Id).ToList()
                            }
                        });
                    ...
                    else
                    {
                        ...
                        return Ok(new CustomResponse<ReportUsersViewModel>
                        {
                            ...
                            Result = new ReportUsersViewModel
                            {
                                ReportUsers = ctx.ReportUsers.Include(x => x.FirstUser).Include(x => x.SecondUser).ToList()
                            }
                        });
        ...
        public async Task<IHttpActionResult> GetReportedPosts(int? User_Id)
        {
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    ...
                    if (User_Id.HasValue)
                    {
                        ...
                        var reportPosts = ctx.ReportPosts.Include(x => x.Post.User).Include(x => x.Post).Include(x => x.User).Where(x => x.User_Id == User_Id).ToList();

                        foreach (var item in reportPosts)
                        {
                            if (item.Post.User == null)
                                item.Post.User = ctx.Users.FirstOrDefault(x => x.Id == item.Post.User_Id);
                            if (item.User == null)
                                item.User = ctx.Users.FirstOrDefault(x => x.Id == item.User_Id);

                        }

                        return Ok(new CustomResponse<ReportPostsViewModel>
                        {
                            Message = Global.ResponseMessages.Success,
                            StatusCode = (int)HttpStatusCode.OK,
                            Result = new ReportPostsViewModel
                            {
                    ...
                    else
                    {
                        ...
                        var reportPosts = ctx.ReportPosts.Include(x => x.Post).Include(x => x.User).ToList();
        ...
        [BasketApi.Authorize("SubAdmin", "SuperAdmin", "ApplicationAdmin")]
        [HttpGet]
        [Route("GetReportedPostById")]
        public async Task<IHttpActionResult> GetReportedPostById(int ReportPost_Id)
        {
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    ...
                    var ReportPost = ctx.ReportPosts.Include(x => x.Post).Include(x => x.Post.Medias).Include(x => x.Post.PollOptions).Include(x => x.Post.User).Include(x => x.User).FirstOrDefault(x => x.Id == ReportPost_Id);


                    ReportPost.Post.LikesCount = ctx.Posts.Sum(p => p.Likes.Where(x => x.Post_Id == ReportPost.Post.Id && x.IsDeleted == false).Count());
                    ReportPost.Post.CommentsCount = ctx.Posts.Sum(p => p.Comments.Where(x => x.Post_Id == ReportPost.Post.Id && x.IsDeleted == false).Count());
                    ReportPost.Post.ShareCount = ctx.Posts.Sum(p => p.Shares.Where(x => x.Post_Id == ReportPost.Post.Id && x.IsDeleted == false).Count());

                    return Ok(new CustomResponse<ReportPostViewModel>
                    {
                        Message = Global.ResponseMessages.Success,
                        StatusCode = (int)HttpStatusCode.OK,
                        Result = new ReportPostViewModel
                        {
        ...
        public async Task<IHttpActionResult> GetAllGroups()
        {
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    ...
                    List<Group> groups = ctx.Groups.Include(x => x.GroupCategories).Include(x => x.User).ToList();

                    foreach(var g in groups)
                    {
                        foreach(var gc in g.GroupCategories)
                        {
                            gc.InterestName = ctx.Interests.Where(x => x.Id == gc.Interest_Id).FirstOrDefault().Name;
                        }
                    }

                    return Ok(new CustomResponse<GroupListViewModel>
                    {
                        Message = Global.ResponseMessages.Success,
                        StatusCode = (int)HttpStatusCode.OK,
        ...
        [BasketApi.Authorize("SubAdmin", "SuperAdmin", "ApplicationAdmin")]
        [HttpPost]
        [Route("ChangeGroupStatuses")]
        public async Task<IHttpActionResult> ChangeGroupStatuses(ChangeGroupStatusListBindingModel model)
        {
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    ...
                    {
                        ctx.Groups.FirstOrDefault(x => x.Id == group.GroupId).IsDeleted = group.Status;
                        ctx.SaveChanges();
                        //var query = string.Empty;
                        //switch (user.Status)
                        //{
        ...
        [HttpPost]
        [Route("AddFAQs")]
        public async Task<IHttpActionResult> AddFAQs(FaqBindingModel model)
        {
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    ...
                    {
                        var FAQ = ctx.FAQ.FirstOrDefault(x => x.Id == model.Id);
        ...
        [HttpGet]
        [Route("GetAllFAQs")]
        public async Task<IHttpActionResult> GetAllFAQs(int? Id)
        {
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    if (!Id.HasValue)
                    {
                        return Ok(new CustomResponse<List<FAQ>>
                        {
                            Message = Global.ResponseMessages.Success,
                            StatusCode = (int)HttpStatusCode.OK,
                            Result = ctx.FAQ.Where(x => !x.IsDeleted).ToList()
                        });
                    }
                    else
                    {
                        return Ok(new CustomResponse<FAQ>
                        {
                            Message = Global.ResponseMessages.Success,
                            StatusCode = (int)HttpStatusCode.OK,
                            Result = ctx.FAQ.FirstOrDefault(x => x.Id == Id.Value)
                        });
                    }
                }
            }
        ...
        [BasketApi.Authorize("SubAdmin", "SuperAdmin", "ApplicationAdmin")]
        [HttpPost]
        [Route("ChangePostStatuses")]
        public async Task<IHttpActionResult> ChangePostStatuses(ChangePostStatusListModel model)
        {
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    ...
                    foreach (var post in model.Posts)
                    {
                        ...
                        var query = @"update Likes SET IsDeleted='1' Where Post_Id=" + post.UserId + @"
update Media SET IsDeleted='" + post.Status + "' Where Post_Id=" + post.UserId + @"
update Comments SET IsDeleted='" + post.Status + "' Where Post_Id=" + post.UserId + @"
update Posts SET IsDeleted='" + post.Status + "' Where Id=" + post.UserId + @"";
                        ctx.Database.ExecuteSqlCommand(query);
                    }
                }
        ...
        public async Task<IHttpActionResult> GetAllInterests()
        {
            try
            {
                using (RiscoContext ctx = new RiscoContext())
                {
                    var data = ctx.Interests.Where(x => !x.IsDeleted).Include(x => x.ParentInterest).ToList();
                    return Ok(new CustomResponse<List<Interest>>
                    {
                        Message = Global.ResponseMessages.Success,
                        StatusCode = (int)HttpStatusCode.OK,
                        Result = data.Where(c => !c.ParentInterestId.HasValue || c.ParentInterestId <= 0)
                            .Select(c => new Interest()
                            {
                                Id = c.Id,
                                Name = c.Name,
                                ParentInterestId = c.ParentInterestId,
                                ChildInterests = GetChildren(data, c.Id)
                            })
                            .ToList()
                    });
        ...
        public static List<Interest> GetChildren(List<Interest> interests, int parentId)
        {
            return interests
                    .Where(c => c.ParentInterestId == parentId)
                    .Select(c => new Interest
                    {
                        Id = c.Id,
                        Name = c.Name,
                        ParentInterestId = c.ParentInterestId,
                        ChildInterests = GetChildren(interests, c.Id)
                    })
                    .ToList();
        }
