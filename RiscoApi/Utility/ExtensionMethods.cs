...
namespace BasketApi
{
    public static class ExtensionMethods
    {
        ...
        public static void CalculatePollPercentage(this Post post)
        {
            foreach (var option in post.PollOptions)
            {
                option.Percentage = Math.Floor((Convert.ToDouble(option.Votes) / Convert.ToDouble(post.TotalVotes)) * 100);
            }
        }

        public static void CheckPollExpiry(this Post post)
        {
            if (DateTime.UtcNow> post.PollExpiryTime)
                post.IsExpired = true;
        }
        ...
        public static void GetPlatformData(this UserProfiles UserProfile, string UserAgent)
        {
            if (!string.IsNullOrEmpty(UserAgent))
            {
                var UserDataArray = UserAgent.Split('/');
